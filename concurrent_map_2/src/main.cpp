...
public:
  ...
  WriteAccess operator[](const K& key) {
    size_t bucket_number = hasher(key) % bucket_count_;
    lock_guard<mutex> g(map_mutexes[bucket_number]);
    MapType& bucket = buckets[bucket_number];
    if (Has(key)) {
  ...
  ReadAccess At(const K& key) const {
    size_t bucket_number = hasher(key) % bucket_count_;
    // lock_guard<mutex> g(map_mutexes[bucket_number]);
    const MapType& bucket = buckets[bucket_number];
    if (Has(key)) {
      return {bucket.at(key), lock_guard<mutex>(value_mutexes[bucket_number])};
    }
    else {
      throw out_of_range("");
    }
  ...
  bool Has(const K& key) const {
    size_t bucket_number = hasher(key) % bucket_count_;
    return buckets[bucket_number].count(key);
  }
  ...
  MapType BuildOrdinaryMap() const {
    ...
    for (size_t i = 0; i < bucket_count_; ++i) {
      ...
      const MapType& bucket = buckets[i];
...
private:
  ...
  mutable vector<mutex> map_mutexes;
  mutable vector<mutex> value_mutexes;
};
