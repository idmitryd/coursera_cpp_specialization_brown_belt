...
Node LoadNode(istream& input) {
  ...
  for (string line; getline(input, line) && Lstrip(line)[1] != '/'; ) {
    ...
    // node_name = <spend; attrs = amount="2500" category="food"></spend>
    attrs = Split(attrs, '>').first;
    // attrs = amount="2500" category="food"
    unordered_map<string, string> node_attrs;
    while (!attrs.empty()) {
      auto [head, tail] = Split(attrs, ' ');
      // head = amount="2500"; tail = category="food"
    ...
    //  Chield имеет в своих аттрибутах два поля amount - 2500, category - food
  }
  return root;
}
