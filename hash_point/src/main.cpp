...
struct Hasher {
  size_t operator()(const Point3D& p3d) const {
    ...
    size_t prime_number = 1451;
    size_t hash_x = cthash(p3d.x);
    size_t hash_y = cthash(p3d.y);
    size_t hash_z = cthash(p3d.z);
    return hash_x * prime_number * prime_number + hash_y * prime_number + hash_z;
  }
};
