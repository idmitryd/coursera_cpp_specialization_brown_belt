...
struct AddressHasher {
  size_t operator()(const Address& address) const {
    size_t prime_number = 5861;
    hash<string> shash;
    hash<int> inthash;
    return shash(address.city) * prime_number * prime_number + 
           shash(address.street) * prime_number + 
           inthash(address.building);
  }
};
...
struct PersonHasher {
  size_t operator()(const Person& p) const {
    ...
    return adhash(p.address) * prime_number * prime_number * prime_number +
           shash(p.name) * prime_number * prime_number +
           inthash(p.height) * prime_number + 
           dhash(p.weight);
  }
};
